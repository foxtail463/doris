...
static void _report_query_profiles_function(
        std::unordered_map<TUniqueId,
                           std::tuple<TNetworkAddress,
                                      ...
                                      std::unordered_map<int, std::vector<TFragmentInstanceStatistics>>>>
                profile_copy,
    ...
    //              {fragment_id -> list<fragment_instance_statistics>}}
    for (auto& entry : profile_copy) {
        ...
        auto& fragment_id_to_instance_statistics = std::get<2>(entry.second);
        ...
        TReportExecStatusParams req = RuntimeQueryStatisticsMgr::create_report_exec_status_params(
                ...
                std::move(fragment_id_to_instance_statistics), std::move(load_channel_profiles),
                /*is_done=*/true);
        TReportExecStatusResult res;
...
TReportExecStatusParams RuntimeQueryStatisticsMgr::create_report_exec_status_params(
        ...
        std::unordered_map<int32_t, std::vector<TFragmentInstanceStatistics>>
                fragment_id_to_instance_statistics,
    ...
    std::map<int32_t, std::vector<TFragmentInstanceStatistics>>
            fragment_id_to_instance_statistics_req;
    for (auto& entry : fragment_id_to_instance_statistics) {
        int32_t fragment_id = entry.first;
        std::vector<TFragmentInstanceStatistics>& instance_statistics = entry.second;
        for (auto& statistics : instance_statistics) {
            statistics.__set_is_done(is_done);
        }
        fragment_id_to_instance_statistics_req[fragment_id] = std::move(instance_statistics);
    }
    profile.__set_fragment_id_to_instance_statistics(fragment_id_to_instance_statistics_req);
...
void RuntimeQueryStatisticsMgr::register_fragment_profile(
        ...
        std::vector<TFragmentInstanceStatistics> instance_statistics,
    ...
    std::unordered_map<int, std::vector<TFragmentInstanceStatistics>>&
            fragment_id_to_instance_statistics =
            std::get<2>(_profile_map[query_id]);
    fragment_id_to_instance_statistics.insert(
            std::make_pair(fragment_id, std::move(instance_statistics)));
