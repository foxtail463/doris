...
private:
    // MemTable 的状态：ACTIVE（活跃写入中）、WRITE_FINISHED（写入完成，等待刷新）、FLUSH（正在刷新到磁盘）
    std::atomic<MemType> _mem_type;
    // Tablet ID（标识这个 MemTable 属于哪个 Tablet）
    int64_t _tablet_id;
    // 是否启用 Unique Key Merge-On-Write 模式
    bool _enable_unique_key_mow = false;
    // 是否刷新成功（用于判断 MemTable 是否已成功刷新到磁盘）
    bool _is_flush_success = false;
    // 部分更新模式：UPSERT（更新或插入）、UPDATE（仅更新）
    UniqueKeyUpdateModePB _partial_update_mode {UniqueKeyUpdateModePB::UPSERT};
    // 键类型：AGG_KEYS（聚合键）、UNIQUE_KEYS（唯一键）、DUPLICATE_KEYS（重复键）
    const KeysType _keys_type;
    // Tablet Schema（表结构定义）
    std::shared_ptr<TabletSchema> _tablet_schema;

    // 行比较器（用于排序和去重，比较 Block 中的行）
    std::shared_ptr<RowInBlockComparator> _vec_row_comparator;

    // 资源上下文（用于资源管理和限制）
    std::shared_ptr<ResourceContext> _resource_ctx;

    // 内存跟踪器（用于监控 MemTable 的内存使用）
    std::shared_ptr<MemTracker> _mem_tracker;
    // 内存分配器（Arena）：只有会被插入到 Block 的行才从 _arena 分配内存
    // 这样可以更准确地计算 MemTable::memory_usage()，最终减少当前导入生成的 segment 文件数量
    ...
    // 加载内存限制（用于控制 MemTable 的内存使用上限）
    int64_t _load_mem_limit = -1;

    // 根据 SlotDescriptor 初始化列偏移量（用于快速访问 Block 中的列）
    void _init_columns_offset_by_slot_descs(const std::vector<SlotDescriptor*>* slot_descs,
                                            const TupleDescriptor* tuple_desc);
    // 列偏移量数组（列索引到 Block 中实际列位置的映射）
    std::vector<int> _column_offset;

    // MemTable 统计信息：插入到 MemTable 的行数
    // 注意：这不是 MemTable 中的实际行数，因为在 Unique Key 或 Aggregate Key 模型中，行可能会被合并
    ...
    // 向量化相关
    //for vectorized
    // 输入可变 Block（存储原始插入的数据）
    vectorized::MutableBlock _input_mutable_block;
    // 输出可变 Block（存储处理后的数据，用于刷新到磁盘）
    vectorized::MutableBlock _output_mutable_block;
    // 最后排序位置（用于增量排序，只对新增的行进行排序）
    size_t _last_sorted_pos = 0;
    // 最后聚合位置（用于增量聚合，只对新增的行进行聚合）
    ...
    // 是否是首次插入（用于判断是否需要初始化聚合函数等）
    bool _is_first_insertion;

    // 初始化聚合函数（根据 Block 的列类型创建对应的聚合函数）
    void _init_agg_functions(const vectorized::Block* block);
    // 聚合函数列表（用于 Aggregate Key 模型的聚合计算）
    std::vector<vectorized::AggregateFunctionPtr> _agg_functions;
    // 聚合状态的偏移量数组（用于快速定位每个聚合函数的状态在内存中的位置）
    std::vector<size_t> _offsets_of_aggregate_states;
    // 聚合状态的总大小（所有聚合函数状态占用的总内存大小）
    size_t _total_size_of_aggregate_states;
    // Block 中的行索引列表（用于排序和聚合，每个元素指向 _input_mutable_block 中的一行）
    std::unique_ptr<DorisVector<std::shared_ptr<RowInBlock>>> _row_in_blocks;

    // 列数（Block 中的列数量）
    size_t _num_columns;
    // 序列列在 Block 中的索引（用于 Unique Key 模型的去重，选择最新版本）
    int32_t _seq_col_idx_in_block {-1};
    // 跳过位图列在 Block 中的索引（用于灵活部分更新，标记哪些列未指定）
    int32_t _skip_bitmap_col_idx {-1};
    // 删除标记列在 Block 中的索引（用于标记删除，DELETE 操作）
