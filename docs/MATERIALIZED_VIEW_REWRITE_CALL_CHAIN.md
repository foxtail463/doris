...
**重写（Rewrite）** 是指查询优化器自动将用户查询转换为使用物化视图的等价查询，从而利用预计算结果加速查询。

#### 重写的本质

重写就是将：
- **原始查询**：从基表（Base Table）读取数据并计算
- **转换为**：从物化视图（Materialized View）读取预计算结果

#### 重写示例

**场景 1：简单查询重写**

假设有一个销售表 `sales`，包含大量历史数据：
...
SELECT 
    date,
    product_id,
    total_amount  -- 直接使用物化视图的预计算结果，无需重新计算 SUM(amount)
FROM mv_daily_sales
WHERE date >= '2025-01-01' AND date <= '2025-01-31';
```

**重写的好处**：
- ✅ **性能提升**：不需要扫描整个 `sales` 表，只需要扫描物化视图（数据量更小）
- ✅ **计算减少**：不需要重新计算 `SUM(amount)`，直接使用预计算结果
- ✅ **响应更快**：查询响应时间从秒级降低到毫秒级
...
SELECT 
    ...
    customer_name,
    total_amount
FROM mv_order_customer
WHERE order_date >= '2025-01-01';
```

**重写的好处**：
- ✅ **避免 JOIN 操作**：不需要执行两个大表的 JOIN，直接读取预计算的 JOIN 结果
- ✅ **减少 I/O**：不需要读取两个表的数据，只需要读取一个物化视图
- ✅ **提升性能**：JOIN 是昂贵的操作，使用物化视图可以显著提升性能

#### 重写的工作原理

1. **查询解析**：用户提交 SQL 查询
2. **物化视图匹配**：优化器检查是否有物化视图可以用于加速查询
3. **等价性验证**：验证使用物化视图的查询结果与原始查询等价
4. **查询重写**：将原始查询转换为使用物化视图的等价查询
5. **执行优化后的查询**：执行重写后的查询，获得更快的响应

#### 重写的关键点

1. **等价性保证**：重写后的查询必须与原始查询产生相同的结果
2. **自动透明**：用户无需修改 SQL，优化器自动完成重写
3. **性能优化**：通过使用预计算结果，显著提升查询性能
4. **智能匹配**：优化器会智能判断哪些查询可以使用哪些物化视图
...
**预重写（Pre-Rewrite）** 是物化视图重写的一个特殊阶段，发生在 RBO（基于规则的优化）阶段之后、CBO（基于代价的优化）阶段之前。

**核心思想**：
- 在正式的 CBO 优化之前，先"试探性"地执行一次优化
- 通过代价模型找到使用物化视图的最佳物理计划
- 从最佳物理计划中"反推"出对应的逻辑计划结构
- 将这个逻辑计划结构作为后续 CBO 优化的输入

**为什么需要预重写？**
1. **提前识别物化视图**：在 CBO 阶段之前就知道哪些物化视图可能被使用
2. **优化输入质量**：为 CBO 阶段提供更好的输入（已经考虑了物化视图的逻辑计划）
3. **处理复杂规则**：某些优化规则（如 TOP N 下推、LIMIT 下推等）需要在物化视图重写之前应用

**预重写 vs 正式重写**：
- **预重写**：在 RBO 之后、CBO 之前，目的是"试探"和"准备"
- **正式重写**：在 CBO 阶段，基于预重写的结果进行实际的查询重写

**类比理解**：
- 预重写 = 做菜前的"试味"：先尝一下食材，确定烹饪方向
- 正式重写 = 正式烹饪：按照试味的结果，正式做菜
...
PreMaterializedViewRewriter.needRecordTmpPlanForRewrite()
  ├─> 检查是否有候选物化视图
  ├─> 检查重写策略配置（FORCE_IN_RBO / TRY_IN_RBO / NOT_IN_RBO）
  └─> 返回是否需要记录临时计划
```

**判断条件**：
- 存在候选物化视图（单表或多表）
- 重写策略允许（不是 NOT_IN_RBO）
- 有物化视图相关的 hook
...
PreMaterializedViewRewriter.rewrite()
  ├─> Optimizer.execute()                    // 步骤1：执行优化器，生成候选物理计划
  ├─> NereidsPlanner.chooseBestPlan()        // 步骤2：根据代价模型选择最佳物理计划
  ├─> MaterializedViewUtils.getChosenMaterializationAndUsedTable()
  │   └─> 步骤3：从最佳物理计划中提取被选中的物化视图和使用的表
  └─> StructInfo.getOriginalPlan()           // 步骤4：提取对应的逻辑计划结构
```

**详细流程**：

1. **执行优化器**：
   - 对 RBO 阶段生成的临时计划应用 CBO 规则
   - 生成多个候选物理计划（可能包含使用物化视图的计划）

2. **选择最佳物理计划**：
   - 使用代价模型评估所有候选计划
   - 选择代价最低的物理计划（可能已经使用了物化视图）
...
4. **提取逻辑计划结构**：
   - 根据使用的表集合，从 memo 中提取对应的逻辑计划结构（StructInfo）
   - 这个逻辑计划结构包含了物化视图的使用信息

5. **返回逻辑计划**：
   - 返回提取的逻辑计划，作为后续 CBO 优化的输入
   - 如果无法提取或没有物化视图被选中，返回 null
...
`AbstractMaterializedViewRule.doRewrite()` 的完整调用链如下：
...
NereidsPlanner.optimize()
  ...
  └─> Optimizer.execute()
      │
      └─> OptimizeGroupExpressionJob.execute()
          │
          ├─> getExplorationRules()           // 获取探索规则列表（包含物化视图规则）
          │   └─> RuleSet.MATERIALIZED_VIEW_IN_CBO_RULES
          │       └─> MaterializedViewFilterScanRule.INSTANCE
          │       └─> MaterializedViewAggregateRule.INSTANCE
          │       └─> MaterializedViewJoinRule.INSTANCE
          │       └─> ... (其他物化视图规则)
          │
          ├─> 为每个规则创建 ApplyRuleJob
          ...
          └─> ApplyRuleJob.execute()
              ...
              │   └─> 匹配规则模式到 GroupExpression
              │
              ├─> rule.transform(plan, cascadesContext)  // 调用规则转换
              │   │
              │   └─> Rule 内部执行（通过 buildRules() 定义的 thenApplyMultiNoThrow 回调）
              │       │
              │       └─> MaterializedViewFilterScanRule.buildRules()
              │           └─> logicalFilter(...).thenApplyMultiNoThrow(ctx -> {
              │                   return rewrite(root, ctx.cascadesContext);
              │               })
              │
              └─> AbstractMaterializedViewRule.rewrite()  // ⭐ 这里被调用
                  │
                  └─> doRewrite()                         // ⭐ 最终调用 doRewrite()
```

**关键点**：
1. **规则注册**：物化视图规则在 `RuleSet.MATERIALIZED_VIEW_IN_CBO_RULES` 中注册
2. **规则应用**：`OptimizeGroupExpressionJob` 遍历所有规则，为每个规则创建 `ApplyRuleJob`
3. **模式匹配**：`ApplyRuleJob` 使用 `GroupExpressionMatching` 匹配规则模式
4. **规则转换**：匹配成功后，调用 `rule.transform()`，这会触发 `buildRules()` 中定义的 `thenApplyMultiNoThrow` 回调
5. **重写执行**：回调中调用 `rewrite()` 方法，最终调用 `doRewrite()`
